...
export class PlanControle {
    ...
    public async create(req: Request, res: Response) {
        try {
            const planCreateDTO = plainToInstance(PlanCreateDTO, req.body);
            const errors = await validate(planCreateDTO);

            if (errors.length > 0) {
                return res.status(400).json({ errors });
            }

            await this.planService.create(planCreateDTO);
    ...
    public async searchById(req: Request, res: Response) {
        const id = Array.isArray(req.params.id)? req.params.id[0] : req.params.id
        try {
            const plan = await this.planService.searchById(id);
            if (plan) {
            ...
            res.status(500).json({ error: error?.message || 'Error searching plan by ID' })
        }
    }
}